...
```
torch==2.10.0 can't be installed because it doesn't have a wheel for the current platform
```

**Orsak:** Versionen saknade stöd för Intel macOS.

**Lösning:** Använd `torch==2.2.2`

**Reflektion:** Stora ML-bibliotek som PyTorch kräver att rätt binära version ("wheel") finns tillgänglig för operativsystemet.
...
**Lösning:** Begränsa NumPy-versionen: `numpy<2.0`

**Reflektion:** När stora bibliotek gör en major-versionändring tar det tid innan hela ekosystemet blir kompatibelt.
...
**Lösning:** Använd `pandas<3.0`

**Reflektion:** Dependency conflicts är vanliga i ML-miljöer och visar varför versionskontroll är viktig.
...
**Reflektion:** PyTorch distribueras ibland via egna index för att rätt CPU/GPU-version ska installeras.
...
**Lösning:** Jag valde rätt interpreter manuellt: `.venv/bin/python`

**Reflektion:** Reproducerbarhet handlar inte bara om installation utan även om att utvecklingsmiljön är korrekt kopplad till rätt venv.
...
Denna laboration visar vikten av noggrant versionskontrollerade miljöer i ML-projekt. Genom att använda moderna verktyg som `uv` och `pyproject.toml` kan vi skapa reproducerbara miljöer som fungerar konsekvent över tid och plattformar.